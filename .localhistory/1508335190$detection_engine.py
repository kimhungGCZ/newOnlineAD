...
#    loop.close()
    ...
    def filter_function_z(array, point, range, len_array):
        if point > range and point < (len_array - range):
            return max(array[range(point-range, point + range)])
        elif point < range:
            return max(array[range(0, 2*range)])
        else:
            return max(array[range(len_array-2*range, len_array - 1)])

    normal_index = [i for i, value in enumerate(result_dta['anomaly_score']) if i > limit_size and i < data_length - limit_size and
                    filter_function_z(result_dta['anomaly_score'],i,limit_size,data_length) >0]
