...
Y_test = np.array([ridge_1.predict(X_test.reshape(-1,1)), ridge_2.predict(X_test.reshape(-1,1)), ridge_3.predict(X_test.reshape(-1,1))]).T
Y_train = raw_data[len(raw_data) - 100:]
print(Y_test)
ridge_conf = Ridge(alpha=1,fit_intercept =True, normalize=True)
