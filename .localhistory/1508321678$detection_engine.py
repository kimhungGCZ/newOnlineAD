...
#    loop.close()
    ...
    def find_inverneghboor_of_point_blocking(index_ano):
        ...
        tree_in = nb.KDTree(X, leaf_size=50)
        ...
        while flag_stop <= int(limit_size/2):
            ...
            dist, ind = tree_in.query([anomaly_point], k=flag_round)
                ...
                if [index_dist, i] not in inverse_neighboor:
                    if inverse_neighboor.size != 0:
                        if i not in inverse_neighboor_temp:
                            in_dist, in_ind = tree_in.query([X[i]], k=flag_round)
            ...
            if len(inverse_neighboor) > limit_size:
                break
        print("Find invert neighbor {}th Time: {} - return value {}".format(index_ano, time.time()-start_time_S, inverse_neighboor));
