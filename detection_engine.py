...
def online_anomaly_detection(DATA_FILE):
    potential_anomaly = []
    ...
    split_anomaly_points = split_list(anomaly_index, 4)
    results = pool.map(calculate_Y_value, (
        split_anomaly_points[0], split_anomaly_points[0], split_anomaly_points[0], split_anomaly_points[0]));
    finalSum = sumYResult(results)
...
def calculate_Y_value(anomaly_points, result_dta):
    sssss = time.time();
    ...
    for anomaly_point in anomaly_points:
        if anomaly_point - 1 not in potential_anomaly and anomaly_point - 2 not in potential_anomaly:
            ...
            if len(set(anomaly_neighboor_detect[:, 1]).intersection(potential_anomaly)) == 0:
                ...
                if (result_dta.value.values[anomaly_point] - result_dta.value.values[
            ...
            if (abs(result_dta.value.values[anomaly_point] - result_dta.value.values[
                consider_point]) - median_sec_der - std_sec_der >= 0):
                anomaly_neighboor = np.array(cmfunc.find_inverneghboor_of_point(tree, X, anomaly_point, limit_size),
                                             dtype=np.int32)
                potential_anomaly.extend([x[1] for x in anomaly_neighboor])
    ...
    print("Time to Calculate Y: {}".format(time.time() - sssss))
    return Y
...
if __name__ == '__main__':
    ...
    result_dta = raw_dataframe.copy()
    result_dta = result_dta.assign(anomaly_score=pd.Series(breakpoint_candidates).values)
    ...
    pool = Pool(processes=4)
    temp_result = calculate_Y_value(split_anomaly_points[0],result_dta)

    # map doWork to availble Pool processes
    results = pool.map(calculate_Y_value, (
        [split_anomaly_points[0],result_dta],
        [split_anomaly_points[1],result_dta],
        [split_anomaly_points[2],result_dta],
        [split_anomaly_points[3],result_dta]));
