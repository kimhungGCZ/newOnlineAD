    ...
    for i, offset in enumerate(clusters_separation):
        ...
        for i, (clf_name, clf) in enumerate(classifiers.items()):
            ...
            try:
                ...
                final_al_result.append([clf_name, temp_precision_value, temp_recal_value,
                                        calculate_f_score(temp_recal_value, temp_precision_value)])
            except:
                final_al_result.append([clf_name, 0, 0,
                                        0])
...
if __name__== "__main__":
    ...
    detect_final_result_array = []
    for data in data_array:
        print("############################# START AT DATASET: {} ##########################################".format(data))
        try:
            detect_final_result = evaluate_function(data);
            writing_data = [data]
            for i in detect_final_result:
                writing_data.extend([i[1],i[2],i[3]])
            try:
                df_final_result = pd.read_csv(os.path.normpath(
                    'D:/Google Drive/13. These cifre/Data Cleaning/workspace/SVADS_real/' + "final_result" + '.csv'))

                df_final_result = pd.DataFrame([writing_data])
                df_final_result.to_csv(os.path.normpath(
                    'D:/Google Drive/13. These cifre/Data Cleaning/workspace/SVADS_real/' + "final_result" + '.csv'),
