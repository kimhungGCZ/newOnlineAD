...
def main_function(DATA_FILE):
    ...
    whole_change_point = change_points
    change_points = change_points[-3:]
    data_pattern_size = []
    flag_count = 0
    prediction_time = 99999
    last_pattern  = 0
    ...
    if len(change_points) < 3:
        ...
        flag_count = 2
    elif len(change_points) == 3 and len(whole_change_point) == 3 :
        ...
        flag_count = 3
        # flag_remove = 0
        # last_pattern = int((len(raw_data) - change_points[-1]))
        # if last_pattern < int((len(raw_data) - change_points[0])*0.2):
        #     flag_remove = 1
        # if flag_remove == 1:
        #     change_points = whole_change_point[-4:-1]
    ...
    for in_index, in_value in enumerate(data_pattern_size):
        if (in_value[1] < mean_data_pattern * 0.3):
            ...
            del data_predicting_test[in_index]

    ##### Smooth Data Test ###########


    lowess = sm.nonparametric.lowess(data_test_value, np.arange(0,len(data_test_value)), frac=0.9)
    data_test_value = lowess[:, 1]
    # plt.plot(np.arange(0,len(data_test_value)), data_test_value, '+')
    # plt.plot(lowess[:, 0], data_test_value)
    # plt.show()
    ...
    print('sum fit coefficients:{}'.format(sum(fitParams)))
    ...
    if len(change_points) == 3:
        ...
        time_per_day_eachDtaPoint_1 = int(
            time_model_1_to_0[0] * difference_days / difference_distances) - difference_days if (int(
            time_model_1_to_0[0] * difference_days / difference_distances) - difference_days) > 0 else 0
        time_per_day_eachDtaPoint_2 = int(
            time_model_2_to_0[0] * difference_days / difference_distances) - difference_days if (int(
            time_model_2_to_0[0] * difference_days / difference_distances) - difference_days) > 0 else 0
        time_per_day_eachDtaPoint_3 = (
            int(time_model_3_to_0[0] * difference_days / difference_distances) - difference_days) if int(
            time_model_3_to_0[0] * difference_days / difference_distances) - difference_days > 0 else 0
        ...
        prediction_time = fitFunc_3(
            [time_per_day_eachDtaPoint_1, time_per_day_eachDtaPoint_2, time_per_day_eachDtaPoint_3],
            *fitParams)
        print("Time to empty by days: {}".format(prediction_time))
        ...
        print("Estimate time to reach 0 at model 1: {}".format(time_per_day_eachDtaPoint_1))
        print("Estimate time to reach 0 at model 2: {}".format(time_per_day_eachDtaPoint_2))
        prediction_time = fitFunc_2([time_per_day_eachDtaPoint_1, time_per_day_eachDtaPoint_2],
                      *fitParams)
        print("Time to empty by days: {}".format(prediction_time))
    else:
        time_model_1_to_0 = (solve(fitFunc_1(x, *model_params[0]), x))
        ...
        time_per_day_eachDtaPoint_1 = int(
            ...
            time_model_1_to_0[0] * difference_days / difference_distances) - difference_days) > 0 else 0
        print("Estimate time to reach 0 at model 1: {}".format(time_per_day_eachDtaPoint_1))
        prediction_time = fitFunc_1([time_per_day_eachDtaPoint_1],
                      *fitParams)
        print("Time to empty by days: {}".format(prediction_time))
